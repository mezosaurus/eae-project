...
public class MainMenuScript : MonoBehaviour {
	...
	void Update () {
		...
		if( controllerInUse && Input.GetAxis("LSY") == 0)
			controllerInUse = false;

		if( controllerInUse && (Input.GetAxis("LSY") < 0 || Input.GetAxis("LSY") > 0) )
			return;

		// check for change
		if( Input.GetAxis("LSY") < 0 || Input.GetKeyDown(KeyCode.DownArrow) )
		{
		...
		if( Input.GetAxis("LSY") > 0 || Input.GetKeyDown(KeyCode.UpArrow) )
		{
